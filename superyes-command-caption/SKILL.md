---
name: superyes-command-caption
description: Generate concise command captions before running shell, PowerShell, bash, cmd, git, npm, pip, file operation, network download, or other terminal commands. Use when an agent is about to execute any command so it explains the exact target, operation, impact, consequence, uncertainty, and advisory risk level before running the original command. Follows the user's language and defaults to English when unclear; use superyes-command-caption-CN or superyes-command-caption-EN for fixed-language captions.
---

# SuperYes Command Caption

Use this skill whenever you are about to run a terminal command. It is the compatibility/default version of SuperYes Command Caption. It automatically follows the user's language when choosing caption language. For fixed-language workflows, use `superyes-command-caption-CN` or `superyes-command-caption-EN`.
...
Then show only a concise caption by default. Match the user's language when clear; otherwise prefer English:
...
Choose the caption language from the current user-facing conversation:

- If the user writes in Chinese, output Chinese captions.
- If the user writes in English, output English captions.
- If the user mixes Chinese and English, prefer the language used for the instruction sentence.
- If the language is unclear, default to English.
- Do not translate command names, file paths, flags, environment variables, URLs, package names, error codes, or other literal technical tokens.
- For workflows that must always use one language, prefer `superyes-command-caption-CN` for Chinese or `superyes-command-caption-EN` for English.
...
- Keep captions concise. Match the user's language when clear; default to English when unclear. Preserve command names, paths, flags, URLs, and technical terms exactly.
